...
/**
 * @ClassName ScalaDemo-req_skip_2 
 * @Author Holden_—__——___———____————_____Xiao
 * @Create 2021年10月27日14:33 - 周三
 * @Describe 关键页面统计
 */
object req_skip_2 {
  def main(args: Array[String]): Unit = {
    ...
    val okIds: List[(String, String)] = ids.zip(ids.tail)

    //todo create case targetClass
    ...
    //todo 计算分母
    val pageIdToCount: Map[String, Int] = actionRDD.filter(
      action => {
        //init是不包含最后一位也就是7，因为我们计算的时候分母不会取7
        ids.init.contains(action.page_id) //不是目标数据的都过滤掉
      }
    ).map({
      action => {
        (action.page_id, 1)
    ...
    val mapRdd: RDD[(String, List[((String, String), Int)])] = userGroup.mapValues {
      t => {
        ...
        pageFlowIds.filter(
          t => {
            okIds.contains(t) //先过滤掉分子中不ok的数据
          }
        ).map(
          t => (t, 1)
        )
